...
def test_classify_search_intent_recognizes_implementation_lookup() -> None:
  assert classify_search_intent("sqlite index refresh") is SearchIntent.IMPLEMENTATION_LOOKUP


def test_classify_search_intent_recognizes_concept_lookup() -> None:
  assert classify_search_intent("semantic search quality") is SearchIntent.CONCEPT_LOOKUP
...
def test_classify_search_intent_prefers_workflow_over_implementation_terms() -> None:
  assert classify_search_intent("index lifecycle flow") is SearchIntent.WORKFLOW_LOOKUP
