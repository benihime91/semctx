...
def test_resolve_requested_embedding_provider_prefers_model_prefix_over_provider() -> None:
  provider = resolve_requested_embedding_provider(
    provider_name="ollama",
...
def test_get_cached_embeddings_uses_provider_config_cache_namespace(
  ...
  def fake_fetch_embeddings(texts: list[str], model: EmbeddingProviderConfig | str) -> list[list[float]]:
