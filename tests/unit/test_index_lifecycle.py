...
def test_index_lifecycle_init_status_refresh_and_clear(tmp_path: Path) -> None:
  ...
  stale_status = status_index(runtime_settings)
  ...
  assert clear_index(runtime_settings) is True
  assert status_index(runtime_settings).exists is False
...
def test_index_lifecycle_init_indexes_markdown_files(tmp_path: Path) -> None:
  ...
  initial_status = init_index(runtime_settings, fetcher=_fake_fetch_embeddings)
...
def test_index_lifecycle_requires_full_rebuild_when_schema_changes(
  ...
  _set_schema_version(get_index_db_path(runtime_settings.cache_dir), "2")

  rebuild_status = status_index(runtime_settings)
...
def test_index_lifecycle_uses_canonical_target_dir_identity(tmp_path: Path) -> None:
  ...
  init_index(runtime_settings, fetcher=_fake_fetch_embeddings)
  store = IndexStore(get_index_db_path(cache_dir))
  metadata = store.load_metadata()
  ...
  assert status_index(build_runtime_settings(target_dir=tmp_path / "." / "src", cache_dir=cache_dir)).rebuild_required is False
...
def test_index_lifecycle_requires_rebuild_for_legacy_scope_metadata(
  ...
  init_index(runtime_settings, fetcher=_fake_fetch_embeddings)
  _set_metadata_value(get_index_db_path(cache_dir), "target_dir_identity", "")
