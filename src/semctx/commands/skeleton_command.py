...
def build_skeleton_payload(
    runtime_settings: RuntimeSettings, file_path: str
) -> JsonObject:
...
def register_skeleton_command(app: typer.Typer) -> None:
    ...
    def skeleton_command(
        ...
        typer.echo(
            render_output(
                text_output=run_skeleton_command(
                    runtime_settings=runtime_settings, file_path=file_path
                ),
                payload=build_skeleton_payload(
                    runtime_settings=runtime_settings, file_path=file_path
                ),
