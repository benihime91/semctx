...
def build_tree_payload(
  ...
  directories = sorted({parent.as_posix() for entry in entries for parent in entry.relative_path.parents if parent.as_posix() != "."})
  files = []
...
def register_tree_command(app: typer.Typer) -> None:
  ...
  def tree_command(
    ...
    depth_limit: int = typer.Option(2, "--depth-limit", min=0, help="Directory depth limit."),
    include_symbols: bool = typer.Option(True, "--include-symbols/--no-symbols", help="Include parsed symbols."),
    max_tokens: int = typer.Option(20_000, "--max-tokens", min=1, help="Approximate output token budget."),
  ) -> None:
