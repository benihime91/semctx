...
def run_search_code_command(
    ...
    return _build_search_code_outputs(
...
def register_search_code_command(app: typer.Typer) -> None:
    ...
    def search_code_command(
        ...
        try:
            text_output, payload = _build_search_code_outputs(
                runtime_settings, query, top_k, model, target_dir, depth_limit
            )
...
def _render_search_code_matches(matches: list[CodeSearchMatch]) -> str:
    ...
    for match in matches:
        lines.append(
            f"{match.relative_path.as_posix()}:{match.start_line}-{match.end_line} "
            f"score={match.score:.4f} semantic={match.semantic_score:.4f} "
            f"lexical={match.lexical_score:.4f}"
        )
