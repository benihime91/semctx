...
def run_search_identifiers_command(
  ...
  return _build_search_identifier_outputs(runtime_settings, query, top_k, model, target_dir, depth_limit)[0]
...
def build_search_identifiers_payload(
  ...
  return _build_search_identifier_outputs(runtime_settings, query, top_k, model, target_dir, depth_limit)[1]
...
def register_search_identifiers_command(app: typer.Typer) -> None:
  ...
  def search_identifiers_command(
    ...
    try:
      text_output, payload = _build_search_identifier_outputs(runtime_settings, query, top_k, model, target_dir, depth_limit)
