...
def build_indexed_file_rows(
  ...
  return [(record.relative_path, record.mtime_ns, record.size_bytes, record.content_hash) for record in records]
...
def build_embedding_rows(
  ...
  return [(record.embedding_id, record.provider, record.model, record.vector_json) for record in records]
