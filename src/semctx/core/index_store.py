...
class IndexStore(IndexStoreDocumentMixin):
    ...
    def load_metadata(self) -> IndexMetadata | None:
        ...
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT key, value FROM index_metadata WHERE key != 'schema_version' OR value != ''"
            ).fetchall()
    ...
    def load_indexed_files(self) -> tuple[IndexedFileRecord, ...]:
        ...
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT relative_path, mtime_ns, size_bytes, content_hash FROM indexed_files ORDER BY relative_path"
            ).fetchall()
