...
def build_index_ignore_matcher(root_dir: Path) -> IndexIgnoreMatcher:
  ...
  override_rules = tuple(_build_override_rule(pattern) for pattern in override_patterns)
  return IndexIgnoreMatcher(baseline_spec=baseline_spec, override_rules=override_rules)
