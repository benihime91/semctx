...
def walk_directory(
  ...
  if resolved_target_path.is_file():
    ...
    if _is_ignored(relative_path, ignore_matcher):
      return []
    return [FileEntry(absolute_path=resolved_target_path, relative_path=relative_path, depth=0)]
  ...
  _collect_entries(
    ...
    supported_suffixes=(INDEXABLE_SUFFIXES if include_index_text_files else CODE_SUFFIXES),
...
def _collect_entries(
  ...
  for child_path in sorted(current_dir.iterdir(), key=lambda path: (path.is_file(), path.name.lower())):
    relative_path = child_path.relative_to(root_dir)
    if _is_ignored(relative_path, ignore_matcher):
      continue
    if child_path.is_dir():
      if child_path.name in IGNORED_DIRECTORY_NAMES or current_depth >= depth_limit:
        continue
      _collect_entries(
        root_dir,
        child_path,
