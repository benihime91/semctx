...
def plan_refresh(
    ...
    if stored_metadata is None or needs_full_rebuild(
    ...
    changed_paths = tuple(
        sorted(
            relative_path
            for relative_path, current_record in current_by_path.items()
            if stored_by_path.get(relative_path) != current_record
        )
    )
    removed_paths = tuple(
        sorted(
            relative_path
            for relative_path in stored_by_path
