...
@beartype
def build_markdown_sections(lines: list[str], heading_pattern: re.Pattern[str]) -> list[tuple[int, int]]:
  """Build markdown section ranges from heading lines."""
  heading_lines = [line_number for line_number, line in enumerate(lines, start=1) if heading_pattern.match(line.strip())]
