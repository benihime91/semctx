...
def get_cached_embeddings(
  ...
  if missing_texts:
    ...
    if len(fetched_vectors) != len(missing_texts):
      raise ValueError("Embedding fetcher returned an unexpected number of vectors.")
    for text, vector in zip(missing_texts, fetched_vectors, strict=True):
      normalized_vector = _normalize_embedding(vector)
      _write_cached_embedding(_build_cache_path(resolved_cache_dir, text), normalized_vector)
...
def _fetch_vertex_embeddings(
  ...
  for text in texts:
    ...
    if len(response_vectors) != 1:
      raise ValueError("Vertex embedding response length did not match the single input length.")
...
def _extract_embedding_vector(item: object) -> list[float]:
  ...
  embedding = _get_dict_embedding(item) if isinstance(item, dict) else getattr(item, "embedding", None)
