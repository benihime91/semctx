...
def resolve_embedding_provider(
    ...
    resolved_model = (
        model.strip() if isinstance(model, str) else _DEFAULT_MODELS[provider_name]
    )
    ...
    return EmbeddingProviderConfig(
        ...
        env_overrides=(
            get_vertex_env_overrides() if provider_name == "vertex_ai" else {}
        ),
    )
...
def get_vertex_env_overrides() -> dict[str, str]:
    ...
    project = _first_env_value(
        "VERTEXAI_PROJECT", "VERTEX_AI_PROJECT", "GOOGLE_CLOUD_PROJECT"
    )
    location = _normalize_vertex_location(
        _first_env_value("VERTEXAI_LOCATION", "VERTEX_AI_LOCATION", "VERTEX_LOCATION")
    )
...
def resolve_requested_embedding_provider(
    ...
    if raw_model:
        ...
        ) or (default_provider.strip() if isinstance(default_provider, str) else "")
        if not resolved_provider_name:
            raise ValueError(
                "Embedding provider is required when --model does not include provider/model."
            )
