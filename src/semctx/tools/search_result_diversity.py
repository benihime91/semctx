...
@beartype
def diversify_code_matches(matches: list[RankedCodeMatch], top_k: int) -> list[RankedCodeMatch]:
...
def _find_same_file_representative(selected: list[RankedCodeMatch], candidate: RankedCodeMatch) -> RankedCodeMatch | None:
...
def _should_defer_for_file_diversity(matches: list[RankedCodeMatch], index: int, selected: list[RankedCodeMatch]) -> bool:
  ...
  if candidate.relative_path not in selected_paths:
    return False
  return any(contender.relative_path not in selected_paths for contender in matches[index + 1 :] if candidate.score - contender.score <= FILE_DIVERSITY_SCORE_GAP)


def _should_promote_primary_match(selected: list[RankedCodeMatch], candidate: RankedCodeMatch) -> bool:
...
def _is_better_primary_match(candidate: RankedCodeMatch, current: RankedCodeMatch) -> bool:
...
def _fill_remaining_slots(selected: list[RankedCodeMatch], deferred: list[RankedCodeMatch], top_k: int) -> list[RankedCodeMatch]:
