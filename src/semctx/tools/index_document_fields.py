...
@beartype
def build_identifier_fields(relative_path: str, symbol: SymbolInfo, header_lines: tuple[str, ...]) -> tuple[str, str, str, str, str]:
...
def find_chunk_symbol(chunk: TextChunk, symbols: tuple[SymbolInfo, ...]) -> SymbolInfo | None:
...
def build_file_symbol_text(symbols: tuple[SymbolInfo, ...]) -> str:
  ...
  return join_text_parts(*dict.fromkeys(part for symbol in symbols for part in (symbol.signature, symbol.name) if part))
