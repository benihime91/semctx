...
def build_chunk_records(
  ...
  for chunk, vector in zip(chunks, vectors, strict=True):
    path_text, header_text, file_symbol_text, local_symbol_text, body_text = build_chunk_fields(
    ...
    embeddings.append(EmbeddingRecord(embedding_id, provider.provider_name, provider.model, json.dumps(vector)))
...
def build_identifier_records(
  ...
  for symbol, vector in zip(symbols, vectors, strict=True):
    path_text, header_text, symbol_name_text, signature_text, context_text = build_identifier_fields(
      relative_path=relative_path,
      symbol=symbol,
      header_lines=header_lines,
    )
...
@beartype
def build_identifier_indexed_text(relative_path: str, symbol: SymbolInfo, header_lines: tuple[str, ...]) -> str:
  """Build the stored indexed text for one identifier."""
  _, header_text, symbol_name_text, signature_text, context_text = build_identifier_fields(
    relative_path=relative_path,
    symbol=symbol,
    header_lines=header_lines,
  )
  return join_text_parts(header_text, symbol_name_text, signature_text, relative_path, context_text)
