...
def trace_blast_radius(
    ...
    entries = walk_directory(
        root_dir=resolved_root_dir, target_path=".", depth_limit=depth_limit
    )
...
def render_blast_radius(report: BlastRadiusReport) -> str:
    ...
    lines.extend(
        f"- {usage.relative_path.as_posix()}:{usage.line_number} :: {usage.line_text}"
...
@beartype
def _collect_usages(
    entries: list[FileEntry], file_context_path: Path, symbol_name: str
) -> list[BlastRadiusUsage]:
...
@beartype
def _collect_entry_usages(
    entry: FileEntry, symbol_name: str, pattern: re.Pattern[str]
) -> list[BlastRadiusUsage]:
    """Collect symbol usages from one file entry."""
    analysis = analyze_file(entry.absolute_path)
    definition_lines = {
        symbol.line_start for symbol in analysis.symbols if symbol.name == symbol_name
    }
    usages: list[BlastRadiusUsage] = []
    for line_number, line in enumerate(
        entry.absolute_path.read_text().splitlines(), start=1
    ):
