...
def compute_fielded_keyword_score(
    ...
    active_fields = [
        ...
        if weight > 0 and field_texts.get(name, "").strip()
    ]
    if not active_fields:
        return 0.0
    total_weight = sum(weight for _, weight in active_fields)
    weighted_score = sum(
        _compute_field_score(query, query_terms, field_texts[name]) * weight
        for name, weight in active_fields
    )
    return _clamp01(weighted_score / total_weight)


def _compute_field_score(
    query: str, query_terms: tuple[str, ...], field_text: str
) -> float:
    ...
    term_coverage = _get_term_coverage(
        set(query_terms), set(tokenize_search_terms(normalized_field_text))
    )
    return _clamp01(
        term_coverage * 0.85 + _get_phrase_boost(query, normalized_field_text)
    )
