...
def build_status(
    ...
    del store
    indexed_file_count, code_chunk_count, identifier_doc_count = _load_status_counts(
        db_path
    )
    return IndexStatus(
        exists=True,
        stale=refresh_plan.rebuild_required
        or bool(refresh_plan.changed_paths or refresh_plan.removed_paths),
