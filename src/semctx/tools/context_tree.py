...
def get_context_tree(
  ...
  entries = walk_directory(root_dir=root_dir, target_path=target_path, depth_limit=depth_limit)
  render_modes = [
...
@beartype
def _render_tree(root_dir: Path, entries: list, include_headers: bool, include_symbols: bool) -> str:
  """Render one context tree mode."""
  lines = [f"{root_dir.name}/"]
  file_analyses = {entry.relative_path: analyze_file(entry.absolute_path) for entry in entries}
  ...
  for entry in entries:
    ...
    if include_headers and analysis.header_lines:
      ...
      lines.append(f"{_indent(entry.relative_path, extra_levels=1)}header: {header_text}")
    if include_symbols:
      for symbol in analysis.symbols:
        lines.append(f"{_indent(entry.relative_path, extra_levels=1)}{symbol.kind} {symbol.name} [{symbol.line_start}-{symbol.line_end}]")
  return "\n".join(lines)
