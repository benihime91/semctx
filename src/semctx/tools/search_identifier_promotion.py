...
@beartype
def promote_identifier_matches(matches: list[RankedIdentifierMatch], top_k: int, query: str) -> list[RankedIdentifierMatch]:
...
def _apply_identifier_promotion(match: RankedIdentifierMatch, query: str) -> RankedIdentifierMatch:
...
def _is_strong_name_subset(query_terms: tuple[str, ...], name_terms: tuple[str, ...]) -> bool:
