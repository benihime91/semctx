...
def resolve_search_ranking_options(
  ...
  return SearchRankingOptions(
    ...
    min_semantic_score=_normalize_threshold(min_semantic_score, DEFAULT_MIN_SEMANTIC_SCORE),
    min_keyword_score=_normalize_threshold(min_keyword_score, DEFAULT_MIN_KEYWORD_SCORE),
    min_combined_score=_normalize_threshold(min_combined_score, DEFAULT_MIN_COMBINED_SCORE),
...
def compute_keyword_score(
  ...
  term_coverage = _get_term_coverage(unique_query_terms, set(tokenize_search_terms(normalized_doc_text)))
  symbol_coverage = _get_term_coverage(unique_query_terms, set(tokenize_search_terms(symbol_text)))
  phrase_boost = 0.15 if query.strip() and query.strip().lower() in normalized_doc_text.lower() else 0.0
...
def compute_combined_score(
  ...
  return _clamp01((semantic_weight * semantic_component + keyword_weight * keyword_score) / total_weight)
...
def passes_score_thresholds(
  ...
  if require_keyword_match and keyword_score <= 0:
    return False
  return not (max(semantic_score, 0.0) < min_semantic_score or keyword_score < min_keyword_score or combined_score < min_combined_score)
